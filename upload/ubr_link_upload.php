...
$THIS_VERSION = '1.0';         // Version of this file
$UPLOAD_ID = '';               // Initialize upload id

require 'ubr_ini.php';
require 'ubr_lib.php';

if(isset($_GET['about']) && $_GET['about'] == 1){ kak("<u><b>UBER UPLOADER LINK</b></u><br>UBER UPLOADER VERSION =  <b>" . $UBER_VERSION . "</b><br>UBR_LINK_UPLOAD = <b>" . $THIS_VERSION . "<b><br>\n", 1, __LINE__); }
else{
	////////////////////////////////////////////////////////////////////////
	//                           * ATTENTION *
...
if($_CONFIG['log_uploads']){ if(!create_dir($_CONFIG['log_dir'])){ showAlertMessage("<font color='red'>ERROR</font>: Failed to create log_dir: " . $_CONFIG['log_dir'], 1); } }
...
// Show debug message
if($DEBUG_AJAX){ showDebugMessage("Upload ID = $UPLOAD_ID<br>"); }

// Write link file
if(write_link_file($TEMP_DIR, $UPLOAD_ID, $DEBUG_UPLOAD, $DELETE_REDIRECT_FILE, $PURGE_TEMP_DIRS, $PURGE_TEMP_DIRS_LIMIT, $_CONFIG)){ startUpload($UPLOAD_ID); }
...
function write_link_file($temp_dir, $upload_id, $debug_upload, $delete_redirect_file, $purge_temp_dirs, $purge_temp_dirs_limit, $_config){
	...
	if(is_resource($fp)){
		...
		fwrite($fp, "delete_link_file<=>". $delete_redirect_file . "\n");
