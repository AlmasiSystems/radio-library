...
*/
var externalSourceFile_items = '/ajax/weekScheduleGetItems/';	// File called by ajax when changes are loaded from the server(by Ajax).
var externalSourceFile_save = '/ajax/weekScheduleSave/';	// File called by ajax when changes are made to an element
var externalSourceFile_delete = '/ajax/weekScheduleDelete';	// File called by ajax when an element is deleted. Input to this file is the variable "eventToDeleteId=<id>"
var popupWindowUrl = '/schedule/editEvent';	// Called when double clicking on an event. use false if this option is disabled.
...
var initMinutes = 30;	// Used to auto set start time. Example: 15 = auto set start time to 0,15,30 or 45. It all depends on where the mouse is located ondragstart
var snapToMinutes = 30;	// Snap to minutes, example: 5 = allow minute 0,5,10,15,20,25,30,35,40,45,50,55
...
function editEventWindow(e,inputDiv)
{
	...
	var editEvent = window.open(popupWindowUrl + '/season_id/' + seasonID + '/?id=' + inputDiv.id,'editEvent','width=500,height=500,status=no');
	editEvent.focus();
}
...
function setElement_showID(id,show_id)
{
	appointmentProperties[id]['showID'] = show_id;
}
function setElement_altShowID(id,alt_show_id)
{
	appointmentProperties[id]['altShowID'] = alt_show_id;
}
...
function parseItemsFromServer(ajaxIndex)
{
	...
	for(var no=1;no<items.length;no++){
		...
		if(itemsToBeCreated[no]['id']){
			...
			appointmentProperties[newIndex]['showID'] = itemsToBeCreated[no]['showID'];
			appointmentProperties[newIndex]['altShowID'] = itemsToBeCreated[no]['altShowID'];
...
function getItemsFromServer()
{
	...
	//weekSchedule_ajaxObjects[ajaxIndex].requestFile = externalSourceFile_items  + '?year=' + dateStartOfWeek.getFullYear() + '&month=' + (dateStartOfWeek.getMonth()/1+1) + '&day=' + dateStartOfWeek.getDate();	// Specifying which file to get
	weekSchedule_ajaxObjects[ajaxIndex].requestFile = externalSourceFile_items  + '?season_id=' + seasonID + '&year=' + dateStartOfWeek.getFullYear() + '&month=' + (dateStartOfWeek.getMonth()/1+1) + '&day=' + dateStartOfWeek.getDate();	// Specifying which file to get
...
//		appointmentProperties[currentAppointmentDiv.id]['showID'] = '0';
//		appointmentProperties[currentAppointmentDiv.id]['altShowID'] = '0';
...
function saveAnItemToServer_complete(index,oldId)
{
	if(oldId.indexOf('new_')>=0){
		...
		if(!inlineTextAreaEnabled){
			//the line below makes a popupwindow appear right after an event is created
...
function saveAnItemToServer(inputId)
{
	...
	if(!appointmentProperties[inputId]['showID'])appointmentProperties[inputId]['showID']='0';
	if(!appointmentProperties[inputId]['altShowID'])appointmentProperties[inputId]['altShowID']='0';
...
function createNewAppointmentDiv(leftPos,topPos,width,contentHTML,height)
{
	...
	timeDiv.onmousedown = initMoveAppointment;
	timeDiv.style.cursor = 'move';
	...
	span.onmousedown = initMoveAppointment;
