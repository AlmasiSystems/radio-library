...
class My_View_Helper_CreateEventWithUrls {

    public function createEventWithUrls($event, $controller = 'shows') {
		...
		if(isset($event['show_id']) && $event['show_id'] != 'NULL' && $event['show_id'] != NULL){
			...
			if(isset($event['show_name'])){
				$output .= '<a href="/'.$controller.'/view/show_id/' . $event['show_id'] . '">' .   $f->filter($event['show_name']) . '</a><br />';
			}
			if(isset($event['alt_show_id'])){
				...
				$output .= '</strong><br />';	
				$output .= '<a href="/'.$controller.'/view/show_id/' . $event['alt_show_id'] . '">' .   $f->filter($event['alt_show_name']) . '</a><br />';
			}
		} else {
